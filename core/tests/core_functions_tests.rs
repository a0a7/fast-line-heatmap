...
mod core_functions_tests {
    ...
    fn test_decode_polyline() {
        ...
        assert!(
            !coords.is_empty(),
            "decoded coordinates should not be empty"
        );

        // check that all coordinates are valid
        for coord in coords {
            assert!(
                coord[0] >= -90.0 && coord[0] <= 90.0,
                "latitude should be valid"
            );
            assert!(
                coord[1] >= -180.0 && coord[1] <= 180.0,
    ...
    fn test_filter_coordinates_by_bounds_rust() {
        ...
        assert!(
            !filtered.is_empty(),
            "some coordinates should be within bounds"
        );

        // check that all filtered coordinates are within bounds
        for coord in filtered {
            assert!(
                coord[0] >= bounds[0] && coord[0] <= bounds[2],
                "latitude should be within bounds"
            );
            assert!(
                coord[1] >= bounds[1] && coord[1] <= bounds[3],
    ...
    fn test_calculate_track_statistics_rust() {
        ...
        assert!(
            result.is_some(),
            "should return statistics for valid coordinates"
        );
    ...
    fn test_simplify_coordinates_rust() {
        ...
        assert!(
            !simplified.is_empty(),
            "simplified coordinates should not be empty"
        );
        assert!(
            simplified.len() <= VALID_COORDS.len(),
            "simplified should have same or fewer points"
        );

        // test with very low tolerance (should keep all points)
        let minimal_simplify = simplify_coordinates_rust(&VALID_COORDS, 0.0);
        assert_eq!(
            minimal_simplify.len(),
            VALID_COORDS.len(),
            "minimal simplification should keep all points"
        );
    }
    ...
    fn test_merge_nearby_tracks_rust() {
        ...
        assert!(
            merged.len() <= tracks.len(),
            "merged should have same or fewer tracks"
        );
    }
    ...
    fn test_split_track_by_gaps_rust() {
        ...
        assert!(
            split_tracks.len() >= 2,
            "should split into multiple tracks due to large gap"
        );
    }
    ...
    fn test_coordinates_to_polyline_rust() {
        ...
        assert_eq!(
            decoded.len(),
            VALID_COORDS.len(),
            "round-trip should preserve point count"
        );
    }
    ...
    fn test_coordinates_to_geojson_rust() {
        ...
        assert_eq!(
            geojson["properties"]["name"], "test track",
        ...
        assert_eq!(
            coordinates.len(),
            VALID_COORDS.len(),
            "should preserve coordinate count"
        );
    }
    ...
    fn test_find_track_intersections_rust() {
        ...
        if !intersections.is_empty() {
            for (coord, track_indices) in intersections {
                assert!(
                    coord[0] >= -90.0 && coord[0] <= 90.0,
                    "intersection latitude should be valid"
                );
                assert!(
                    coord[1] >= -180.0 && coord[1] <= 180.0,
                    "intersection longitude should be valid"
                );
                assert!(
                    track_indices.len() >= 2,
                    "intersection should involve at least 2 tracks"
                );
            }
        }
    }
    ...
    fn test_calculate_coverage_area_rust() {
        ...
        assert!(
            result.is_some(),
            "should return coverage area for valid tracks"
        );
        ...
        assert!(
            empty_result.is_none(),
            "should return none for empty tracks"
        );
    }
    ...
    fn test_cluster_tracks_by_similarity_rust() {
        ...
        assert!(
            clusters.len() <= tracks.len(),
            "clusters should not exceed track count"
        );

        for (representative, members, score) in clusters {
            assert!(
                !representative.is_empty(),
                "representative track should not be empty"
            );
    ...
    fn test_get_bounding_box_rust() {
        ...
        assert!(
            min_lng <= max_lng,
            "min longitude should be <= max longitude"
        );

        // verify bounds contain all points
        for coord in VALID_COORDS {
            assert!(
                coord[0] >= min_lat && coord[0] <= max_lat,
                "point should be within lat bounds"
            );
            assert!(
                coord[1] >= min_lng && coord[1] <= max_lng,
                "point should be within lng bounds"
            );
        }
    }
    ...
    fn test_resample_track_rust() {
        ...
        if VALID_COORDS.len() <= target_count {
            assert_eq!(
                resampled.len(),
                VALID_COORDS.len(),
                "should return all points when target exceeds input"
            );
        } else {
            assert!(
                resampled.len() <= target_count + 1,
                "should not exceed target count by much"
            );
        }
    }
    ...
    fn test_calculate_distance_between_points() {
        ...
        assert!(
            distance < 1000.0,
            "distance should be reasonable for nearby points"
        );
        ...
        assert!(
            zero_distance < 0.001,
            "distance from point to itself should be near zero"
        );
    }
    ...
    fn test_polyline_encoding_decoding_roundtrip() {
        ...
        for (orig, dec) in original.iter().zip(decoded.iter()) {
            ...
            assert!(
                lat_diff < 0.001,
                "latitude should be preserved within precision"
            );
            assert!(
                lng_diff < 0.001,
                "longitude should be preserved within precision"
            );
        }
    }
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(
            valid_count, 0,
            "empty coordinates should have 0 valid count"
        );
        ...
        assert_eq!(
            extreme_valid, 2,
            "extreme valid coordinates should be accepted"
        );
        assert!(
            extreme_issues.is_empty(),
            "extreme valid coordinates should have no issues"
        );
    }
    ...
    fn test_performance_with_large_dataset() {
        ...
        assert!(
            simplified.len() < large_coords.len(),
            "should actually simplify large dataset"
        );
    }
}
