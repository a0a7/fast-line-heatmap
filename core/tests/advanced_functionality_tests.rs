...
mod comprehensive_functionality_tests {
    ...
    const RWGPS_COORDS: &str =
        r#"[[37.7749,-122.4194],[37.7849,-122.4094],[37.7949,-122.3994],[37.8049,-122.3894]]"#;
    ...
    fn test_polyline_processing() {
        ...
        assert!(
            ...
            "Should return empty for invalid data"
        );
    }
    ...
    fn test_heatmap_generation() {
        ...
        assert!(
            heatmap.max_frequency > 0,
            "Max frequency should be positive"
        );
        assert!(
            heatmap.max_frequency >= 1,
            "Max frequency should be at least 1"
        );

        // Find tracks with maximum frequency (the repeated main street)
        let max_freq_tracks: Vec<_> = heatmap
            .tracks
            .iter()
        ...
        assert!(
            !max_freq_tracks.is_empty(),
            "Should have tracks with max frequency"
        );

        // Verify frequency calculation
        let frequencies: Vec<u32> = heatmap.tracks.iter().map(|t| t.frequency).collect();
        assert!(
            frequencies.iter().any(|&f| f > 1),
            "Should have tracks with frequency > 1"
        );
    }
    ...
    fn test_unrealistic_jump_filtering() {
        ...
        assert!(
            filtered.len() < coords_with_jumps.len(),
        ...
        for window in filtered.windows(2) {
            if let [p1, p2] = window {
                ...
                assert!(
                    distance <= 100.0,
                    "No consecutive points should be > 100km apart"
                );
            }
        }
    }
    ...
    fn test_fit_file_detection() {
        ...
        assert!(
            is_fit_file(&valid_fit_header),
            "Should detect valid FIT file"
        );
    ...
    fn test_track_simplification_precision() {
        ...
        assert!(
            simplified_aggressive.len() < simplified_conservative.len(),
            "Aggressive simplification should reduce more points"
        );
        assert!(
            simplified_conservative.len() <= dense_track.len(),
            "Conservative simplification should not add points"
        );

        // Should always keep first and last points
        assert_eq!(
            simplified_aggressive[0], dense_track[0],
            "Should keep first point"
        );
        assert_eq!(
            simplified_aggressive.last(),
            dense_track.last(),
            "Should keep last point"
        );
    }
    ...
    fn test_intersection_detection_accuracy() {
        ...
        assert_eq!(
            track_indices.len(),
            2,
            "Intersection should involve 2 tracks"
        );
    ...
    fn test_coverage_area_calculation() {
        ...
        assert!(
            (min_lng - (-122.5000)).abs() < 0.001,
            "Min lng should match"
        );
        assert!(
            (max_lng - (-122.4000)).abs() < 0.001,
            "Max lng should match"
        );
    }
    ...
    fn test_track_clustering_similarity() {
        ...
        for (representative, members, score) in &clusters {
            assert!(
                ...
                "Representative should not be empty"
            );
    ...
    fn test_geojson_coordinate_order() {
        ...
        assert_eq!(
            first_coord[0].as_f64().unwrap(),
            -122.4194,
            "First element should be longitude"
        );
        assert_eq!(
            first_coord[1].as_f64().unwrap(),
            37.7749,
            "Second element should be latitude"
        );
    }
    ...
    fn test_snap_to_grid_consistency() {
        ...
        assert!(
            (snapped1[0] - grid_lat).abs() < f64::EPSILON,
            "Should be aligned to grid"
        );
        assert!(
            (snapped1[1] - grid_lng).abs() < f64::EPSILON,
            "Should be aligned to grid"
        );
    }
    ...
    fn test_track_resampling_accuracy() {
        ...
        assert_eq!(
            upsampled.len(),
            original_track.len(),
            "Should not add points when upsampling"
        );
    }
    ...
    fn test_performance_benchmarks() {
        ...
        assert!(
            simplify_time.as_millis() < 100,
            "Simplification should be fast"
        );
        assert!(
            bbox_time.as_millis() < 10,
            "Bounding box calculation should be very fast"
        );
        ...
        assert!(
            !simplified.is_empty(),
            "Simplification should produce results"
        );
        assert_eq!(bbox.len(), 4, "Bounding box should have 4 values");
        assert_eq!(
            resampled.len(),
            1001,
            "Resampling should hit target (+1 for last point)"
        );
